...
def _pg_conninfo():
    # DATABASE_URL varsa onu kullan; yoksa tek tek değişkenlerden oluştur
...
def db():
    # Her çağrıda kısa ömürlü bağlantı açıyoruz; psycopg automanage
...
def init_db():
    with db() as con:
        with con.cursor() as cur:
            # users & comments tabloları (PostgreSQL tipleri)
            cur.execute("""
            ...
            """)
            # Eski tablolarda eksik kolonlar varsa ekle
...
def login_required(fn):
    ...
    def wrapper(*args, **kwargs):
        if "user_id" not in session:
            flash("Yorum yazmak için giriş yapmalısınız.", "warn")
...
def movie_detail(movie_id):
    detail = tmdb_get(f"/movie/{movie_id}", {"append_to_response": "videos,credits,release_dates"})
    ...
    with db() as con, con.cursor() as cur:
        cur.execute("""
                    SELECT c.id,
                           c.content,
                           c.is_spoiler,
                           c.created_at,                                       -- datetime olarak kalsın
                           to_char(c.created_at AT TIME ZONE 'Europe/Istanbul',
                                   'YYYY-MM-DD HH24:MI:SS') AS created_at_str, -- string yedek
                           c.sentiment_label,
                           c.sentiment_score,
                           u.username
                    FROM comments c
    ...
    return render_template("detail.html",
                           movie=detail, recs=recs, comments=comments,
